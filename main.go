...
func printVersion(c *cli.Command) {
	_, _ = fmt.Fprintf(c.Root().Writer, "%s version %s\n", c.Root().Name, version.FullVersion())
}

func commandNotFound(ctx context.Context, c *cli.Command, command string) {
	_, _ = fmt.Fprintf(c.Root().ErrWriter, "buildkite-agent: unknown subcommand %q\n", command)
	_, _ = fmt.Fprintf(c.Root().ErrWriter, "Run '%s --help' for usage.\n", c.Root().Name)
	os.Exit(1)
}
