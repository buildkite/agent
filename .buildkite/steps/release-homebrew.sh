...
GITHUB_RELEASE_ACCESS_TOKEN="$(aws ssm get-parameter \
...
BASE_FORMULA_FILE="buildkite-agent@${AGENT_VERSION%%.*}.rb"
...
echo "Homebrew formula file: ${BASE_FORMULA_FILE}"
...
if [[ "${DRY_RUN:-}" == "false" ]] ; then
  ...
  curl -X PUT "https://api.github.com/repos/buildkite/homebrew-buildkite/contents/Formula/${BASE_FORMULA_FILE}" \
      -H "Authorization: token ${GITHUB_RELEASE_ACCESS_TOKEN}" \
