...
echo "--- Building alpine package ${PLATFORM}/${ARCH}"
...
# Build the alpine package using the architectre and binary, they are saved to deb/
./scripts/build-alpine-package.sh "$ARCH" "$BINARY" "$AGENT_VERSION" "$BUILD_VERSION"
