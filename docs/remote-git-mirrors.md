...
Status: **implemented on `main`**. The four-PR stack in §6 has landed, along
with production follow-ups (notably C22's presence-only warm hits) and the F3
eligibility relaxation for pull request head builds. This document remains the
living design record: requirements, decisions, caveats and §6's test lists are
kept current as behaviour evolves, while §4 ("ground truth") describes the
pre-stack checkout the design was reasoned against.
...
confirmed `refs/pull/*` replication for Q3). Everything the mirror serves for a
PR build is still the immutable object ID: with default fetch flags,
`refs/pull/N/head` is only ever fetched to *obtain* objects — it writes no
local ref, nothing downstream resolves it, and `verifyCommit` skips PR builds —
so on a mirror hit the canonical PR-ref fetch is skipped entirely, and on a
miss it runs exactly as before, retry semantics included. A PR build without a
known commit (`BUILDKITE_COMMIT=HEAD`) remains ineligible through the
full-object-ID clause. The `--refmap` clause guards the one supported way that
fetch *does* produce durable local names (for example
`--refmap=+refs/pull/*:refs/pull/origin/*`, exercised by
`TestCheckingOutGitHubPullRequestAndCustomRefmap`): a mirror hit would leave
those refs silently stale rather than merely lagging, so such builds stay
canonical. Persisted `remote.origin.fetch` mappings are the config-shaped
...
on-host hit. A default clone's mapping never matches `refs/pull/*`, so the
common case keeps zero canonical contact on a hit — a canonical outage is
invisible to a PR build whose mirror already replicated the commit. Builds using the speculative **merge** ref (`refs/pull/N/merge`)
remain skipped: the merge commit is regenerated by the provider rather than
pinned to the build's object ID, and its canonical fetch doubles as the
merge-conflict fail-fast, so a mirror hit could mask a conflicted PR. (These
two clauses key on the merge-refspec flag and the fetch flags, not on
`BUILDKITE_PULL_REQUEST` itself; an empty value — a config anomaly the fetch
...
scan (C22); a pull request head build whose commit is reachable only through
`refs/pull/*` hits the mirror with canonical closed, writing the namespaced
ref but never `refs/pull/*`, and a lagging mirror falls back to the canonical
PR-ref fetch; and cancellation after starting mirror work causes no canonical
fallback.
...
any of its keys; a pull request head build whose commit is reachable only
through `refs/pull/*` hits the mirror with canonical closed, and falls back to
the canonical PR-ref fetch on a lagging mirror; a persisted `refs/pull/*`
fetch mapping keeps the canonical PR-ref fetch (and its opportunistic ref
updates) even when the mirror has or served the objects; and —
...
ref. With F3, fork PR builds join this keyspace under their source branch
names, so a fork branch named like a canonical branch can ping-pong the shared
anchor ref across lineages; the exposure stays in C22's accepted class (one
retryable failure). *Comment: in `checkout_mirror.go`.*
...
*Partially answered:* Cursor Origin confirmed `refs/pull/*` refs are mirrored
(2026-08), unlocking F3 for PR head builds; tag replication remains unconfirmed.
