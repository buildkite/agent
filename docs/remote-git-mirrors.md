...
| 12 | Mirror attempt must not consume the whole checkout budget when the mirror hangs (buildsworth on #4144) | Built in: with a configured checkout timeout, a child deadline reserves at least half the attempt budget for same-attempt canonical fallback. Without a configured timeout, `http.lowSpeedLimit`/`http.lowSpeedTime` detect stalled transfers without capping healthy large transfers; curl/OS defaults remain the connect-phase bound. |
