...
Status: **implemented on `main`**. The four-PR stack in §7 has landed, along
...
design record: requirements, decisions and caveats are current, while
plan-tense sections (§4 "ground truth", §7 delivery plan) describe the state
and reasoning at design time.
...
PR build is still the immutable object ID: `refs/pull/N/head` is only ever
fetched to *obtain* objects — it is never written as a local ref, nothing
downstream resolves it, and `verifyCommit` skips PR builds — so on a mirror hit
the canonical PR-ref fetch is skipped entirely, and on a miss it runs exactly
as before, retry semantics included. A PR build without a known commit
(`BUILDKITE_COMMIT=HEAD`) remains ineligible through the full-object-ID clause.
Builds using the speculative **merge** ref (`refs/pull/N/merge`) remain
skipped: the merge commit is regenerated by the provider rather than pinned to
the build's object ID, and its canonical fetch doubles as the merge-conflict
fail-fast, so a mirror hit could mask a conflicted PR.
...
scan (C22); a pull request head build hits the mirror with canonical closed,
writing the namespaced ref but never `refs/pull/*`, and a lagging mirror falls
back to the canonical PR-ref fetch; and cancellation after starting mirror
work causes no canonical fallback.
...
any of its keys; a pull request head build hits the mirror with canonical
closed and falls back to the canonical PR-ref fetch on a lagging mirror; and —
