...
var KubernetesBootstrapCommand = &cli.Command{
	...
	Action: func(ctx context.Context, c *cli.Command) error {
		...
		if err := socket.StatusLoop(context.WithoutCancel(ctx), func(err error) {
			...
			// cancel the job.
			if err != nil {
				l.ErrorContext(ctx, "kubernetes-bootstrap: Error waiting for client interrupt; cancelling work", "error", err)
			} else {
				l.WarnContext(ctx, "kubernetes-bootstrap: Either the job was cancelled or the pod is being deleted; cancelling work")
			}
			...
			go func() {
				...
				l.InfoContext(ctx, "kubernetes-bootstrap: Timed out waiting for subprocess to exit; exiting immediately", "status", 1)
				os.Exit(1)
			}()
		}); err != nil {
		...
		go func() {
			for {
				...
				case sig := <-signals:
					...
					l.InfoContext(ctx, "kubernetes-bootstrap: Received signal; awaiting interrupt from agent", "signal", sig)
				}
			}
		}()
