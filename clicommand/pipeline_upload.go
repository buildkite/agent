	...
	Action: func(c *cli.Context) error {
		...
		if cfg.ApplyIfChanged {
			func() {
				...
				applyIfChanged(l, result.Steps, changedPaths)
			}()
		} else {
			stripIfChanged(result.Steps)
		}
...
// applyIfChanged converts "if_changed" into "skip" where the glob matches no
// files in the git diff.
...
// stripIfChanged removes "if_changed" from all steps.
