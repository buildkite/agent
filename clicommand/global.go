...
var (
	...
	GitSparseCheckoutModeFlag = &cli.StringFlag{
		...
		Sources: cli.EnvVars("BUILDKITE_GIT_SPARSE_CHECKOUT_MODE"),
	}
	...
	GitFetchBaseBranchFlag = &cli.StringFlag{
		...
		Usage:            fmt.Sprintf("Also fetch the base branch during checkout, so every command in the job can diff against its current tip; one of %v. ′optimistic′ warns and carries on when the fetch fails, ′strict′ fails the job, and ′off′ (default) skips the fetch. A transient failure is retried on the same budget as the job's own fetch, and a base branch the remote does not have fails without retrying. The base branch is the first non-empty value of {$BUILDKITE_PULL_REQUEST_BASE_BRANCH, $BUILDKITE_PIPELINE_DEFAULT_BRANCH}; nothing is fetched when it is the branch being built, and under ′strict′ a job with neither set fails", gitFetchBaseBranchModes),
...
func setupLoggerAndConfig[T any](ctx context.Context, c *cli.Command, opts ...configOpts) (
	...
	if err != nil {
		_, _ = fmt.Fprintf(c.Root().ErrWriter, "%s\n", err)
		os.Exit(1)
	}
