...
SANITIZED_AGENT_NAME=$(echo "$BUILDKITE_AGENT_NAME" | tr -cs '[:alnum:]' '_')
...
if [[ -e "$BUILDKITE_HOOKS_PATH/checkout" ]]; then
  ...
  if [[ ! -z "${BUILDKITE_SSH_FINGERPRINT_VERIFICATION:-}" ]] && [[ "$BUILDKITE_SSH_FINGERPRINT_VERIFICATION" == "true" ]]; then
