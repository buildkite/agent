...
func TestFetchSecrets_Success(t *testing.T) {
	...
	secrets, errs := FetchSecrets(t.Context(), slog.New(slog.DiscardHandler), apiClient, "test-job-id", []string{"DATABASE_URL", "API_TOKEN"}, 10)
	if len(errs) > 0 {
...
func TestFetchSecrets_EmptyKeys(t *testing.T) {
	...
	apiClient := api.NewClient(slog.New(slog.DiscardHandler), api.Config{Endpoint: server.URL, Token: "llamas"})
	secrets, errs := FetchSecrets(t.Context(), slog.New(slog.DiscardHandler), apiClient, "test-job-id", []string{}, 10)
...
func TestFetchSecrets_NilKeys(t *testing.T) {
	...
	secrets, errs := FetchSecrets(t.Context(), slog.New(slog.DiscardHandler), apiClient, "test-job-id", nil, 10)
...
func TestFetchSecrets_SomeSecretsFail(t *testing.T) {
	...
	secrets, errs := FetchSecrets(t.Context(), slog.New(slog.DiscardHandler), apiClient, "test-job-id", keys, 10)
...
func TestFetchSecrets_APIClientError(t *testing.T) {
	...
	secrets, errs := FetchSecrets(t.Context(), slog.New(slog.DiscardHandler), apiClient, "test-job-id", keys, 10, noSleep)
...
func TestFetchSecrets_RetriesOnServerError(t *testing.T) {
	...
	secrets, errs := FetchSecrets(t.Context(), slog.New(slog.DiscardHandler), apiClient, "test-job-id", []string{"MY_SECRET"}, 10, noSleep)
	if len(errs) > 0 {
...
func TestFetchSecrets_NoRetryOnNonRetryableStatus(t *testing.T) {
	...
	secrets, errs := FetchSecrets(t.Context(), slog.New(slog.DiscardHandler), apiClient, "test-job-id", []string{"MISSING"}, 10, noSleep)
	if len(errs) != 1 {
		t.Fatalf("expected 1 error, got %d: %v", len(errs), errs)
	}
