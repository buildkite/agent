...
func newLockServer(logger *slog.Logger) *lockServer {
	return &lockServer{
		logger: logger,
		locks:  newLockState(),
	}
...
func (s *lockServer) getLock(w http.ResponseWriter, r *http.Request) {
	...
	if key == "" {
		if err := socket.WriteError(w, "key missing", http.StatusNotFound); err != nil {
			s.logger.ErrorContext(r.Context(), "Agent API: couldn't write error", "error", err)
		}
		return
	}
	resp := &ValueResponse{
	...
	if err := json.NewEncoder(w).Encode(resp); err != nil {
		s.logger.ErrorContext(r.Context(), "Agent API: couldn't encode response body", "error", err)
	}
}
