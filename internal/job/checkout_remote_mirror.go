...
func (e *Executor) resolveRemoteMirrorAttempt(previousAttempts int) remoteMirrorAttempt {
	...
	if e.PullRequestUsingMergeRefspec {
		// The speculative merge ref is regenerated by the provider rather
		// than pinned to an immutable object ID, and its canonical fetch
		// doubles as the merge-conflict fail-fast: a mirror hit could mask a
		// conflicted PR. Keep merge-ref builds canonical (see the follow-ups
		// section of docs/remote-git-mirrors.md).
		return skip(remoteMirrorSkipPullRequestMergeRef)
	}
	if e.PullRequest != "false" {
		// PR head builds are otherwise mirror-eligible: the build is
