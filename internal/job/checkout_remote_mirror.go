...
func (e *Executor) resolveRemoteMirrorAttempt(previousAttempts int) remoteMirrorAttempt {
	...
	if e.PullRequest != "false" && e.PullRequestUsingMergeRefspec {
		// The speculative merge ref is regenerated by the provider rather than
		// pinned to an immutable object ID, and its canonical fetch doubles as
		// the merge-conflict fail-fast. Keep merge-ref builds canonical (F3).
