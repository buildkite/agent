...
func TestCheckingOutLocalGitProjectWithSparseCheckout_WithGitMirrors(t *testing.T) {
	...
	git.ExpectAll([][]any{
		...
		{"--git-dir", matchSubDir(tester.GitMirrorsDir), "config", "gc.auto", "0"},
		{"--version"},
		{"clone", "-v", "--filter=blob:none", "--sparse", "--reference", matchSubDir(tester.GitMirrorsDir), "--dissociate", "--", tester.Repo.Path, "."},
		{"clean", "-fdq"},
...
func TestCheckingOutLocalGitProjectWithSubmodules_WithGitMirrors(t *testing.T) {
	...
	git.ExpectAll([][]any{
		...
		{"-c", "protocol.file.allow=always", "submodule", "update", "--init", "--force", "--reference", matchSubDir(tester.GitMirrorsDir), "--dissociate", "--", filepath.Base(submoduleRepo.Path)},
