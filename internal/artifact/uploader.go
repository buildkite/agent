...
func (t *artifactUploadTimings) logSummary(l *slog.Logger) {
	l.Info(fmt.Sprintf(
...
func (a *Uploader) Upload(ctx context.Context) error {
	...
	batchCreator := NewArtifactBatchCreator(a.logger, a.apiClient, BatchCreatorConfig{
		...
		OnBatchCreated: func(count int, duration time.Duration) {
			...
			a.logger.Debug(fmt.Sprintf("Created %d artifacts in %s", count, duration))
		},
	})
	for chunk, err := range batchCreator.Batches(ctx) {
		...
		a.logger.Debug(fmt.Sprintf("Uploaded %d artifact work units with %d workers in %s", stats.workUnits, stats.workerCount, stats.uploadDuration))
	}
...
func (a *Uploader) collect(ctx context.Context) ([]*api.Artifact, error) {
	...
	a.logger.Debug(fmt.Sprintf("Artifact path discovery finished in %s", time.Since(searchStart)))

	drainStart := time.Now()
	wg.Wait()
	a.logger.Debug(fmt.Sprintf("Artifact collection workers drained in %s", time.Since(drainStart)))
...
func (a *Uploader) upload(ctx context.Context, artifacts []*api.Artifact, uploader workCreator) (artifactUploadBatchStats, error) {
	...
	a.logger.Debug(fmt.Sprintf("Uploading %d artifact work units with %d workers", workUnitCount, workerCount))
...
func (a *artifactUploadWorker) updateStates(ctx context.Context) error {
	...
	a.logger.Debug(fmt.Sprintf("Updated %d artifact states in %s", len(statesToUpload), duration))
	return nil
}
