...
func TestKeyPartUnmarshal(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "agent commit", yaml: `{ agent: commit }`, want: KeyPart{Source: SourceAgent, Arg: "commit"}},
...
func TestKeyPartResolve(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "agent commit", part: KeyPart{Source: SourceAgent, Arg: "commit"}, env: map[string]string{"BUILDKITE_COMMIT": "0123456789abcdef"}, want: "0123456789abcdef"},
