...
func TestSaveWithClient_Error(t *testing.T) {
	...
	err := saveWithClient(ctx, logger.Discard, mock, []string{"cache1"}, 1)
	if err == nil {
		t.Fatalf("err error = %v, want non-nil error", err)
	}
	if want := "failed to save cache"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
	if want := "save failed"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
}
...
func TestRestoreWithClient_Error(t *testing.T) {
	...
	if want := "failed to restore cache"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
	if want := "restore failed"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
}
...
`
	...
	if got, want := fileConfig.Dependencies, 2; len(got) != want {
		t.Fatalf("len(fileConfig.Dependencies) = %v, want %d", got, want)
	}
...
`
	...
	if want := "failed to unmarshal cache config file"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
}
...
func TestLoadCacheConfiguration_FileNotFound(t *testing.T) {
	...
	if want := "failed to read cache config file"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
}
...
func TestLoadCacheConfiguration_EmptyFile(t *testing.T) {
	...
	if got := len(fileConfig.Dependencies); got != 0 {
		t.Fatalf("len(fileConfig.Dependencies) = %v, want 0", got)
	}
}
...
`
	...
	if want := "cache IDs not found in configuration"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
	if want := "invalid1"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
	if want := "invalid2"; err == nil || !strings.Contains(err.Error(), want) {
		t.Fatalf("err error = %v, want error containing %q", err, want)
	}
}
...
`
	...
	if got := client; got == nil {
		t.Fatalf("client = %v, want non-nil value", err)
	}
	if diff := cmp.Diff(cacheIDs, []string{"cache1", "cache2"}); diff != "" {
		t.Fatalf("cacheIDs diff (-got +want):\n%s", diff)
	}
}
...
`
	...
	if diff := cmp.Diff(cacheIDs, slices.Sorted(slices.Values([]string{"cache1", "cache2"}))); diff != "" {
		t.Fatalf("cacheIDs diff (-got +want):\n%s", diff)
	}
}
