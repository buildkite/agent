...
func (r *JobRunner) createEnvironment(ctx context.Context) ([]string, error) {
	...
	// exception is sparse-checkout paths and the no-cone toggle, which have a
	// from-job floor: a step's checkout.sparse config is honored under the default
	// mode too, and only strict locks them (see IsCheckoutLockedForJobEnv).
	// Within-job sources (hooks, plugins, the Job API) are governed separately and
	// only strict locks them.
...
BUILDKITE_AGENT_JWKS_KEY_ID`
	...
	setCheckoutEnv("BUILDKITE_GIT_SPARSE_CHECKOUT_NO_CONE", fmt.Sprint(r.conf.AgentConfiguration.GitSparseCheckoutNoCone))
