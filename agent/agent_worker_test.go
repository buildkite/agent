...
func TestDisconnect(t *testing.T) {
	...
	if diff := cmp.Diff(rec.Messages(), []string{"Disconnecting...", "Disconnected"}); diff != "" {
...
func TestDisconnectRetry(t *testing.T) {
	...
	if got, want := len(msgs), 4; got != want {
		t.Fatalf("len(rec.Messages()) = %d, want %d", got, want)
	}
	if got, want := msgs[0], "Disconnecting..."; got != want {
		t.Errorf("rec.Messages()[0] = %q, want %q", got, want)
	}
	if got, want := msgs[1], regexp.MustCompile(`POST http.*/disconnect: 500 Internal Server Error \(Attempt 1/4`); !want.MatchString(got) {
		t.Errorf("rec.Messages()[1] = %q, want string matching %v", got, want)
	}
	if got, want := msgs[2], regexp.MustCompile(`POST http.*/disconnect: 500 Internal Server Error \(Attempt 2/4`); !want.MatchString(got) {
		t.Errorf("rec.Messages()[2] = %q, want string matching %v", got, want)
	}
	if got, want := msgs[3], "Disconnected"; got != want {
		t.Errorf("rec.Messages()[3] = %q, want %q", got, want)
	}
}
