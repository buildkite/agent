...
func (a *AgentWorker) runStreamingPingLoop(ctx context.Context, outCh chan<- actionMessage) error {
	...
	for {
		...
		a.logger.DebugContext(ctx, "[runStreamingPingLoop] Waiting for jitter", "duration", jitter, "max_duration", window)
		select {
		case <-skipWait:
			// continue below
...
func (a *streamLoopState) startStream(ctx, streamCtx context.Context) error {
	...
	a.logger.DebugContext(ctx, "[runStreamingPingLoop] Connecting", "attempt", a.attempts)
	...
	if err != nil {
		...
		a.logger.DebugContext(ctx, "[runStreamingPingLoop] Connection to ping stream failed", "error", err)
...
func (a *streamLoopState) handle(ctx context.Context, msg *agentedgev1.StreamPingsResponse, streamErr error) error {
	a.logger.DebugContext(ctx, "[runStreamingPingLoop] Received message", "message", msg, "error", streamErr)
	...
	case streamErr != nil:
		// TODO: after we've made streaming endpoints generally available,
		// think about making some of these logs error or warning level.
		a.logger.DebugContext(ctx, "[runStreamingPingLoop] Connection to ping stream failed or ended", "error", streamErr)
	...
	default:
		...
		case *agentedgev1.StreamPingsResponse_Pause:
			if reason := act.Pause.GetReason(); reason != "" {
				a.logger.InfoContext(ctx, "Agent paused", "reason", reason)
			}
			amsg.action = "pause"

		case *agentedgev1.StreamPingsResponse_Disconnect:
			if reason := act.Disconnect.GetReason(); reason != "" {
				a.logger.InfoContext(ctx, "Agent disconnected", "reason", reason)
			}
