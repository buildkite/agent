...
func (a *AgentWorker) runStreamingPingLoop(ctx context.Context, outCh chan<- actionMessage) error {
	log := a.logger.With("component", "runStreamingPingLoop")
	...
	for {
		...
		log.DebugContext(ctx, "Waiting for jitter", "duration", jitter, "max_duration", window)
		select {
		case <-skipWait:
			// continue below
		...
		log.DebugContext(ctx, "Waiting for remainder of window")
		select {
		case <-skipWait:
...
func (a *streamLoopState) startStream(ctx, streamCtx context.Context) error {
	...
	a.logger.DebugContext(ctx, "Connecting", "attempt", a.attempts)
	...
	if err != nil {
		...
		a.logger.DebugContext(ctx, "Connection to ping stream failed", "error", err)
		if isUnrecoverable(err) {
			a.logger.DebugContext(ctx, "Stopping because the error is unrecoverable")
			return err
		}
		// Fast fallback to the ping loop
		a.logger.DebugContext(ctx, "Becoming unhealthy")
		...
		case a.outCh <- actionMessage{unhealthy: true}:
			a.logger.DebugContext(ctx, "Unhealthy message sent to debouncer")
			// sent!
		case <-a.stop:
	...
	a.logger.DebugContext(ctx, "Waiting for a message...")
...
func (a *streamLoopState) handle(ctx context.Context, msg *agentedgev1.StreamPingsResponse, streamErr error) error {
	a.logger.DebugContext(ctx, "Received message", "message", msg, "error", streamErr)
	...
	case streamErr != nil:
		// TODO: after we've made streaming endpoints generally available,
		// think about making some of these logs error or warning level.
		a.logger.DebugContext(ctx, "Connection to ping stream failed or ended", "error", streamErr)
		...
		if connect.CodeOf(streamErr) == connect.CodeDeadlineExceeded {
			a.logger.DebugContext(ctx, "Breaking stream loop to reconnect following deadline-exceeded")
	...
	case msg == nil:
		a.logger.DebugContext(ctx, "Ping stream yielded a nil message, so assuming the stream is broken")
		a.logger.DebugContext(ctx, "Becoming unhealthy")
		amsg.unhealthy = true
	...
	select {
	case a.outCh <- amsg:
		a.logger.DebugContext(ctx, "Message sent to debouncer")
		// sent!
	case <-a.stop:
		a.logger.DebugContext(ctx, "Stopping due to agent stop")
		return errInternalStop
	...
	if amsg.action == "disconnect" {
		a.logger.DebugContext(ctx, "Stopping due to disconnect action")
		a.internalStop()
		return errInternalStop
	}

	if amsg.unhealthy {
		a.logger.DebugContext(ctx, "Breaking stream loop to reconnect because the stream is unhealthy")
		// Re-arm the "connection established" log so the next successful
		// connection (after this mode switch to the ping loop) logs.
		a.firstMsg = true
		return errInternalBreak
