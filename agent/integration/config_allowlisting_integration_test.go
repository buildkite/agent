...
func evilPluginsJSON(t *testing.T) string {
	t.Helper()

	p := pipeline.Plugins{
		&pipeline.Plugin{
			Source: "github.com/crime-org/super-nasty-plugin#1.0.0",
...
func nicePluginsJSON(t *testing.T) string {
	t.Helper()

	p := pipeline.Plugins{
		&pipeline.Plugin{
			Source: "github.com/buildkite-plugins/docker#v5.9.2",
			Config: map[string]any{
				"some": "config",
			},
		},
	...
	tests := []testCase{
		{
			...
			wantExitStatus:           "-1",
			wantLogsContain:          []string{"denied environment variables: [BASH_ENV]"},
