...
func (t *tagFetcher) Fetch(ctx context.Context, l *slog.Logger, conf FetchTagsConfig) ([]string, error) {
	...
	if conf.TagsFromK8s {
		k8sTags, err := t.k8s()
		if err != nil {
			l.WarnContext(ctx, "Could not fetch tags from Kubernetes", "error", err)
		}
	...
	if conf.TagsFromHost {
		...
		if err != nil {
			l.WarnContext(ctx, "Failed to find hostname", "error", err)
		}
	...
	if conf.TagsFromEC2MetaData {
		l.InfoContext(ctx, "Fetching EC2 meta-data...")
		...
		err := roko.NewRetrier(
			...
			if err != nil {
				l.WarnContext(ctx, "Fetching EC2 metadata failed; retrying", "error", err, "retry", r.String())
			} else {
				l.InfoContext(ctx, "Successfully fetched EC2 meta-data")
		...
		if err != nil {
			...
			l.ErrorContext(ctx, "Failed to fetch EC2 meta-data", "error", err)
		}
	}
	...
	if len(conf.TagsFromEC2MetaDataPaths) > 0 {
		...
		if err != nil {
			l.ErrorContext(ctx, "Error parsing meta-data tag and path pairs", "error", err)
		}
	...
	if conf.TagsFromEC2Tags {
		l.InfoContext(ctx, "Fetching EC2 tags...")
		...
		err := roko.NewRetrier(
			...
			if err != nil {
				l.WarnContext(ctx, "Fetching EC2 tags failed; retrying", "error", err, "retry", r.String())
			} else {
				l.InfoContext(ctx, "Successfully fetched EC2 tags")
				for tag, value := range ec2Tags {
					tags = append(tags, fmt.Sprintf("%s=%s", tag, value))
				}
				r.Break()
		...
		if err != nil {
			...
			l.ErrorContext(ctx, "Failed to find EC2 tags", "error", err)
		}
	}

	// Attempt to add the default ECS meta-data tags
	if conf.TagsFromECSMetaData {
		l.InfoContext(ctx, "Fetching ECS meta-data...")
		...
		err := roko.NewRetrier(
			...
			if err != nil {
				l.WarnContext(ctx, "Fetching ECS metadata failed; retrying", "error", err, "retry", r.String())
			} else {
				l.InfoContext(ctx, "Successfully fetched ECS meta-data")
		...
		if err != nil {
			...
			l.ErrorContext(ctx, "Failed to fetch ECS meta-data", "error", err)
		}
	}

	// Attempt to add the default GCP meta-data tags
	if conf.TagsFromGCPMetaData {
		l.InfoContext(ctx, "Fetching GCP meta-data...")
		...
		err := roko.NewRetrier(
			...
			if err != nil {
				...
				l.ErrorContext(ctx, "Failed to fetch Google Cloud meta-data", "error", err)
				return err
			}
		...
		if err != nil {
			...
			l.ErrorContext(ctx, "Failed to fetch GCP meta-data", "error", err)
		}
	}
	...
	if conf.TagsFromGCPLabels {
		l.InfoContext(ctx, "Fetching GCP instance labels...")
		err := roko.NewRetrier(
			roko.WithMaxAttempts(5),
			...
			if err != nil {
				l.WarnContext(ctx, "Fetching GCP instance labels failed; retrying", "error", err, "retry", r.String())
			} else {
				l.InfoContext(ctx, "Successfully fetched GCP instance labels")
		...
		if err != nil {
			...
			l.ErrorContext(ctx, "Failed to find GCP instance labels", "error", err)
		}
	}
