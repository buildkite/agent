...
func (a *AgentWorker) runDebouncer(ctx context.Context, bat *baton[string], outCh chan<- actionMessage, inCh <-chan actionMessage) error {
	...
	// We begin holding the baton, so release it when we're done. (Re-releasing
	// it is a no-op).
	defer func() {
	...
	for {
		...
		case <-iif(healthy, bat.Acquire(actorDebouncerLoop)): // if the stream is healthy, take the baton
		...
		case msg, open := <-inCh: // streaming loop has produced an event
			...
			// Yes, we're healthy. Can we send this message right away?
			// We must have the baton, and an action must not be in progress.
			haveBaton := bat.Holder() == actorDebouncerLoop
			if !(haveBaton && !actionInProgress) {
				// No, the ping loop is currently in possession of the toggle.
				// Debounce messages until we have it.
				a.logger.Debug("[runDebouncer] Debouncing (action %q, jobID %q) while waiting (haveBaton = %t, actionInProgress = %t)", msg.action, msg.jobID, haveBaton, actionInProgress)
			...
			// We're clear to pass this message on to the action handler right
			// away.
