...
func TestFetchingTags(t *testing.T) {
	...
	tags, err := (&tagFetcher{}).Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg)
	if err != nil {
		t.Fatalf("(&tagFetcher{}).Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg) error = %v, want nil", err)
	}
...
func TestFetchingTagsWithHostTags(t *testing.T) {
	...
	if got, want := tags, "llamas"; !slices.Contains(got, want) {
		t.Errorf("(&tagFetcher{}).Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg) = %v, want containing %q", got, want)
	}
...
func TestFetchingTagsFromEC2(t *testing.T) {
	...
	tags, err := fetcher.Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg)
	if err != nil {
		t.Fatalf("fetcher.Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg) error = %v, want nil", err)
	}
	...
	if diff := cmp.Diff(slices.Sorted(slices.Values(tags)), slices.Sorted(slices.Values([]string{"llamas", "rock", "aws:instance-id=i-blahblah", "aws:instance-type=t2.small", "custom_tag=true"}))); diff != "" {
		t.Errorf("fetcher.Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg) sorted diff (-got +want):\n%s", diff)
	}
}
...
func TestFetchingTagsFromAllSources(t *testing.T) {
	...
	if got, want := tags, "llamas"; !slices.Contains(got, want) {
		t.Errorf("fetcher.Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg) = %v, want containing %q", got, want)
	}
	if got, want := tags, "rock"; !slices.Contains(got, want) {
		t.Errorf("fetcher.Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg) = %v, want containing %q", got, want)
	}
...
func TestFailOnMissingTags_EC2MetaData(t *testing.T) {
	...
	if diff := cmp.Diff(tags, []string{"llamas"}); diff != "" {
		t.Errorf("fetcher.Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg) diff (-got +want):\n%s", diff)
	}
	...
	_, err = fetcher.Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg)
	if err == nil {
		t.Fatalf("fetcher.Fetch(t.Context(), slog.New(slog.DiscardHandler), cfg) error = %v, want non-nil error", err)
	}
