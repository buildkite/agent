...
func (r *JobRunner) Cancel(reason CancelReason) error {
	...
	r.agentLogger.Info(fmt.Sprintf("Canceling job %s with a signal grace period of %v (%s)",
		r.conf.Job.ID,
	...
	case <-time.After(r.conf.AgentConfiguration.CancelSignalTimeout):
		r.agentLogger.Info(fmt.Sprintf("Job %s hasn't stopped within %v, terminating",
			r.conf.Job.ID,
			r.conf.AgentConfiguration.CancelSignalTimeout),
		)
