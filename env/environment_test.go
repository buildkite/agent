...
func TestEnvironmentSet_NormalizesKeyNames(t *testing.T) {
	...
	default:
		...
		if got, want := camel, "Cerro Torre"; got != want {
			t.Errorf("camel = %q, want %q", got, want)
		}
...
func TestEnvironmentApply(t *testing.T) {
	...
	if diff := cmp.Diff(FromSlice([]string{
		"LLAMAS_ENABLED=1",
	}).Dump(), env.Dump()); diff != "" {
		t.Errorf("FromSlice([]string{\n\t\"LLAMAS_ENABLED=1\",\n}).Dump() diff (-got +want):\n%s", diff)
	}
	...
	if diff := cmp.Diff(FromSlice([]string{}).Dump(), env.Dump()); diff != "" {
		t.Errorf("FromSlice([]string{}).Dump() diff (-got +want):\n%s", diff)
	}
}
